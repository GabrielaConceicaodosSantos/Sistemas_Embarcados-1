...
void Display::Print(char *word) {
	if ((strlen(word) - 1) > 32) {
		...
		//Command("Go to first line"); //volte para o inicio da primeira linha
		for (unsigned int i = 0; i < (strlen(word)); i++) {
			/*if (i == 16) {
			 Command("Activate second line");
			 Command("Go to second line");
			 Command("LCD ON, Cursor ON, Cursor blinking ON");
			 ...
			 */
			Send_Data(word[i]); //enviar os dados para os pinos de dados
			Selection('d'); //selecao para dados
			Enable(); //Habilitar
			//if (i != 15) {
			Command("Increment cursor");
...
//Funcao que recebe os comandos a serem aplicados no display
	if (command_name == "Increment cursor") {
		...
		Send_Data(0x01); //envia os dados do comando
		Selection('c'); //atualiza o pino selecionando o modo de comando
		Enable(); //Habilita o comando
		...
		Selection('c');
		Enable();
	} else if (command_name == "Go to first line") {
		Send_Data(0x80);
		Selection('c');
		Enable();
	} else if(command_name == "Display ON ,Cursor blinking OFF"){
		Send_Data(0x0E);
		Selection('c');
		Enable();
	}
